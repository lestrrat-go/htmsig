...
# Use GITHUB_REF, but if the ref is v1, keep it as v1
my $link_ref = $ENV{GITHUB_REF} || "v1";
...
for my $filename (@files) {
    ...
    for my $line (<$src>) {
        ...
        $output->print("source: [$include_filename](https://github.com/lestrrat-go/htmsig/blob/$link_ref/$include_filename)\n");
